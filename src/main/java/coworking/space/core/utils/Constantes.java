package coworking.space.core.utils;

public class Constantes {

    public Constantes( ) {
        throw new IllegalStateException( "Utility class" );
    }

    public static final Object DELETED_BY_ID = "Test deleted by ID.";
    public static final String ERROR_OBTENER_SALAS = "Error al obtener las salas: ";
    public static final String ERROR_CREAR_SALA = "Error al crear la sala: ";
    public static final String ERROR_ACTUALIZAR_SALA = "Error al actualizar la sala: ";
    public static final String ERROR_INACTIVAR_SALA = "Error al eliminar(INACTIVAR) la sala: ";
    public static final String ERROR_CAMBIAR_ESTADO_SALA = "Error al obtener la sala: ";

}